...
- **`rls(x, d, Ntaps, λ)`**: Recursive Least Squares (RLS) algorithm with forgetting factor λ
- **`rls_apriori(x, d, Ntaps, λ)`**: RLS algorithm using the a priori error formulation with forgetting factor λ
...
The LMS, NLMS, LMS-Newton, RLS, and CMA functions return the filtered output, final filter coefficients, squared error history, and the evolution of filter coefficients over time.
